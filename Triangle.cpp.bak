...
#ifdef ANSI_DECLARATORS
int reconstruct(struct mesh *m, struct behavior *b, int *triList,
                REAL *triangleattriblist, REAL *triAreaList,
                int elements, int corners, int attribs,
                int *segmentList,int *segMarkList, int numOfSegments)
#else /* not ANSI_DECLARATORS */
int reconstruct(m, b, triList, triangleattriblist, triAreaList,
                elements, corners, attribs, segmentList, segMarkList,
                numOfSegments)
struct mesh *m;
struct behavior *b;
int *triList;
...

{
  ...
  while (triangleloop.tri != (triangle *) NULL) {
...
#ifdef TRILIBRARY
      killvertexindex = triList[vertexindex++];
#else /* not TRILIBRARY */
      stringptr = findfield(stringptr);
      if (*stringptr != '\0') {
    ...
    if (b->vararea) {
#ifdef TRILIBRARY
      area = triAreaList[elementnumber - b->firstnumber];
#else /* not TRILIBRARY */
  ...
  if (b->poly) {
    ...
    while (subsegloop.ss != (subseg *) NULL) {
      ...
      if (segmentmarkers) {
        boundmarker = segMarkList[segmentnumber - b->firstnumber];
      }
#else /* not TRILIBRARY */
...
void formskeleton(struct mesh *m, struct behavior *b, int *segmentList,
                  int *segMarkList, int numOfSegments)
#else /* not ANSI_DECLARATORS */
void formskeleton(m, b, segmentList, segMarkList, numOfSegments)
struct mesh *m;
struct behavior *b;
int *segmentList;
int *segMarkList;
...
void transfernodes(struct mesh *m, struct behavior *b, REAL *pointList,
                   REAL *pointattriblist, int *pointMarkList,
                   int numOfPoints, int numberofpointattribs)
#else /* not ANSI_DECLARATORS */
void transfernodes(m, b, pointList, pointattriblist, pointMarkList,
                   numOfPoints, numberofpointattribs)
struct mesh *m;
struct behavior *b;
REAL *pointList;
...

{
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    if (pointMarkList != (int *) NULL) {
      /* Read a vertex marker. */
      setvertexmark(vertexloop, pointMarkList[i]);
    } else {
...
void writenodes(struct mesh *m, struct behavior *b, REAL **pointList,
                REAL **pointattriblist, int **pointMarkList)
#else /* not ANSI_DECLARATORS */
void writenodes(m, b, pointList, pointattriblist, pointMarkList)
struct mesh *m;
struct behavior *b;
REAL **pointList;
REAL **pointattriblist;
...

{
  ...
  if (!b->nobound && (*pointMarkList == (int *) NULL)) {
    *pointMarkList = (int *) trimalloc((int) (outvertices * sizeof(int)));
  }
  plist = *pointList;
...
void writeelements(struct mesh *m, struct behavior *b,
                   int **triList, REAL **triangleattriblist)
#else /* not ANSI_DECLARATORS */
void writeelements(m, b, triList, triangleattriblist)
struct mesh *m;
struct behavior *b;
int **triList;
REAL **triangleattriblist;
...

{
  ...
  if (*triList == (int *) NULL) {
    *triList = (int *) trimalloc((int) (m->triangles.items *
                                             ((b->order + 1) * (b->order + 2) /
                                              2) * sizeof(int)));
  }
...
void writepoly(struct mesh *m, struct behavior *b,
               int **segmentList, int **segMarkList)
#else /* not ANSI_DECLARATORS */
void writepoly(m, b, segmentList, segMarkList)
struct mesh *m;
struct behavior *b;
int **segmentList;
int **segMarkList;
...

{
  ...
  if (!b->nobound && (*segMarkList == (int *) NULL)) {
    *segMarkList = (int *) trimalloc((int) (m->subsegs.items *
                                                  sizeof(int)));
  }
  slist = *segmentList;
  smlist = *segMarkList;
...
void writeedges(struct mesh *m, struct behavior *b,
                int **edgeList, int **edgeMarkList)
#else /* not ANSI_DECLARATORS */
void writeedges(m, b, edgeList, edgeMarkList)
struct mesh *m;
struct behavior *b;
int **edgeList;
int **edgeMarkList;
...

{
  ...
  if (!b->nobound && (*edgeMarkList == (int *) NULL)) {
    *edgeMarkList = (int *) trimalloc((int) (m->edges * sizeof(int)));
  }
  elist = *edgeList;
  emlist = *edgeMarkList;
  index = 0;
...

{
...
#ifdef TRILIBRARY
  transfernodes(&m, &b, in->pointList, in->pointAttrList,
                in->pointMarkList, in->numOfPoints,
                in->numOfPointAttrs);
#else /* not TRILIBRARY */
...
#ifdef TRILIBRARY
    m.hullsize = reconstruct(&m, &b, in->triList,
                             in->triAttrList, in->triAreaList,
                             in->numOfTriangles, in->numOfCorners,
                             in->numOfTriAttributes,
                             in->segmentList, in->segMarkList,
                             in->numOfSegments);
#else /* not TRILIBRARY */
  ...
  if (b.usesegments) {
    ...
    if (!b.refine) {
      ...
      formskeleton(&m, &b, in->segmentList,
                   in->segMarkList, in->numOfSegments);
#else /* not TRILIBRARY */
  ...
  if (b.poly && (m.triangles.items > 0)) {
    ...
    m.holes = in->numOfHoles;
    regionarray = in->regionList;
    m.regions = in->numOfRegions;
#else /* not TRILIBRARY */
...
#ifdef TRILIBRARY
  if (b.jettison) {
    out->numOfPoints = m.vertices.items - m.undeads;
  } else {
    out->numOfPoints = m.vertices.items;
  }
  out->numOfPointAttrs = m.nextras;
  out->numOfTriangles = m.triangles.items;
  out->numOfCorners = (b.order + 1) * (b.order + 2) / 2;
  out->numOfTriAttributes = m.eextras;
  out->numOfEdges = m.edges;
  if (b.usesegments) {
    out->numOfSegments = m.subsegs.items;
  } else {
    out->numOfSegments = m.hullsize;
  }
  if (vorout != (struct triangulateio *) NULL) {
    vorout->numOfPoints = m.triangles.items;
    vorout->numOfPointAttrs = m.nextras;
    vorout->numOfEdges = m.edges;
  }
#endif /* TRILIBRARY */
...
#ifdef TRILIBRARY
    writenodes(&m, &b, &out->pointList, &out->pointAttrList,
               &out->pointMarkList);
#else /* not TRILIBRARY */
...
#ifdef TRILIBRARY
    writeelements(&m, &b, &out->triList, &out->triAttrList);
#else /* not TRILIBRARY */
...
#ifdef TRILIBRARY
      writepoly(&m, &b, &out->segmentList, &out->segMarkList);
      out->numOfHoles = m.holes;
      out->numOfRegions = m.regions;
      if (b.poly) {
  ...
  if (b.edgesout) {
#ifdef TRILIBRARY
    writeedges(&m, &b, &out->edgeList, &out->edgeMarkList);
#else /* not TRILIBRARY */
  ...
  if (b.voronoi) {
#ifdef TRILIBRARY
    writevoronoi(&m, &b, &vorout->pointList, &vorout->pointAttrList,
